...
# ==========================================
# 🛑 COMMON FORCE JOIN MESSAGE HELPER (With Local Photo)
# ==========================================
...
def send_welcome(message):
    ...
    if not is_joined_all:
        remove_verified_user(user_id) # Agar live me nahi mila toh file se bhi uda do
...
def get_player_info(message):
    ...
    # ⚡ 1. LIVE PREMIUM FORCE JOIN CHECKER (TUMHARI DEMAND PAR) ⚡
    # Ab bot har command par live puchega ki banda sach me joined hai ya leave kar gaya!
    is_joined_all = False
    try:
        valid_statuses = ['member', 'administrator', 'creator']
        status_grp = bot.get_chat_member(GROUP_USERNAME, user_id).status
        ...
        is_joined_all = False # Agar API error aayi, tab bhi access deny karega

    if not is_joined_all:
        remove_verified_user(user_id) # Usne cheat kiya toh file se bhi kick mar diya!
        send_force_join_msg(message)
        return
...
├─ Fire Pass: {bp_pass}
├─ Current Bp Badges: {basic.get('badgeCnt', 0)}
├─ Br Rank: {br_rank_str}
├─ Cs Rank: {cs_rank_str}
├─ Gender: {gender}
├─ Show Rank: {mode_prefer}
├─ Show Br Rank: {basic.get('showBrRank', True)}
├─ Show Cs Rank: {basic.get('showCsRank', True)}
...
└─ Leader Information:
    ...
    ├─ Leader Current Bp Badges: {captain.get('badgeCnt', 0)}
    ├─ Leader Br Rank: {leader_br_str}
